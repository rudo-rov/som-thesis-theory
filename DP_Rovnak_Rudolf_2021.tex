...
data type \texttt{int32_t} -- therefore 32 bit signed integer number. Overflow and underflow is not
addressed in my implementation, therefore the behaviour copies that of the C++ type.
...
The behaviour of explicit return in blocks is where the term non--local return comes in. 
...
As per scoping rules defined in SOM, code blocks can access local variables of method evaluating them. However, every frame stack holds
